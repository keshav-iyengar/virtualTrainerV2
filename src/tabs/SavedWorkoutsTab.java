		...
		for(int i = 1; i < recordId; i++) {
			...
			linkSavedWorkout.addListener(SWT.Selection, new Listener() {
				public void handleEvent(Event event) {
					yourWorkoutTab.setWorkout(null,
							dbEngine.readFromDB("SELECT * FROM workouts WHERE id = " + String.valueOf(id)));
			...
			linkRmWorkout.addListener(SWT.Selection, new Listener() {
				public void handleEvent(Event event) {
					linkSavedWorkout.setVisible(false);
					linkRmWorkout.setVisible(false);
